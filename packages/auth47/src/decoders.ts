...
const bitcoinAddressMainnetRegex =
  /\b(bc(0([02-9ac-hj-np-z]{39}|[02-9ac-hj-np-z]{59})|1[02-9ac-hj-np-z]{8,87})|[13][1-9A-HJ-NP-Za-km-z]{25,35})\b/;
const bitcoinAddressTestnetRegex =
  /\b(tb(0([02-9ac-hj-np-z]{39}|[02-9ac-hj-np-z]{59})|1[02-9ac-hj-np-z]{8,87})|[2mn][1-9A-HJ-NP-Za-km-z]{25,39})\b/;
...
export function validateGenerateUriArgs(
  args: GenerateURIArgs,
): asserts args is GenerateURIArgs {
  ...
  if (
    args.resource != null &&
    (typeof args.resource !== "string" || args.resource.length === 0)
  ) {
...
function validateBitcoinAddress(address: unknown) {
  ...
  if (
    !bitcoinAddressMainnetRegex.test(address) &&
    !bitcoinAddressTestnetRegex.test(address)
  ) {
