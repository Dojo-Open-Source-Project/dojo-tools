import { BIP47Factory, type TinySecp256k1Interface } from "@dojo-tools/bip47";
...
type TinySecp256k1InterfaceJoined = TinySecp256k1Interface &
  Parameters<typeof bitcoinMessageFactory>[0];
...
const getNetwork = (
  networkString: keyof typeof utils.networks,
): Network | null => {
...
export class Auth47Verifier {
  ...
  verifyProof(
    proof: unknown,
    networkString: keyof typeof utils.networks = "bitcoin",
  ): VerifyResult {
    const network = getNetwork(networkString);

    if (!network) {
      throw new Auth47Error(
        `Invalid network: ${networkString} (expected "bitcoin", "testnet" or "regtest")`,
      );
    }
