...
describe("createCallbackUri", () => {
  ...
  it("should accept valid srbns URLs", () => {
    assert.doesNotThrow(() =>
      createCallbackUri("srbns://123aef4567890aef@samourai.io"),
    );
  });

  it("should throw for URLs with unsupported protocols", () => {
    assert.throws(
      () => createCallbackUri("ftp://example.com"),
      "invalid protocol for callback URI",
    );
  });

  it("should throw for URLs with hash", () => {
    assert.throws(
      () => createCallbackUri("http://example.com#hash"),
      "hash is forbidden in callback URI",
    );
  });

  it("should throw for URLs with search parameters", () => {
    assert.throws(
      () => createCallbackUri("http://example.com?param=value"),
      "search params are forbidden in callback URI",
    );
  });
});
...
describe("validateGenerateUriArgs", () => {
  ...
  it("should throw when nonce is missing", () => {
    assert.throws(
      () => validateGenerateUriArgs({} as GenerateURIArgs),
      '"nonce": missing',
    );
  });
  ...
  it("should throw when nonce is not alphanumeric", () => {
    assert.throws(
      () => validateGenerateUriArgs({ nonce: "invalid-nonce!" }),
      '"nonce": invalid, expected alphanumeric string',
    );
  });
  ...
  it("should throw when expires is invalid", () => {
    ...
    assert.throws(
      () => validateGenerateUriArgs({ nonce: "validnonce", expires: pastDate }),
  ...
  it("should accept valid args", () => {
    ...
    assert.doesNotThrow(() =>
      validateGenerateUriArgs({ nonce: "validnonce", expires: futureDate }),
    );
...
describe("validateChallenge", () => {
  ...
  it("should throw for challenges with c parameter", () => {
    assert.throws(
      () => validateChallenge("auth47://aZrzsdfsfs343432sdf?r=srbn&c=value"),
      '"challenge": invalid param "c',
    );
  });
});
...
describe("validateProof", () => {
  ...
  it("should throw when auth47_response is missing", () => {
    assert.throws(
      () => validateProof({}),
      '"auth47_response": missing, expected 1.0',
    );
  });

  it("should throw when auth47_response is not 1.0", () => {
    assert.throws(
      () => validateProof({ auth47_response: "2.0" }),
  ...
  it("should throw when challenge is missing", () => {
    assert.throws(
      () => validateProof({ auth47_response: "1.0" }),
      '"challenge": missing',
    );
  });
