...
import type { AddressType, Network, TinySecp256k1Interface } from "./types.js";
...
export class PaymentCodePublic {
  ...
  constructor(
    ...
    bip32: BIP32API,
    buf: Uint8Array,
    network: Network = utils.networks.bitcoin,
  ) {
    this.ecc = ecc;
    this.bip32 = bip32;
   ...
   * @protected
   */
  protected getAddressFromPubkey(
    pubKey: Uint8Array,
    type: AddressType,
  ): string {
    switch (type) {
      ...
      default: {
        throw new Error(
          `Unknown address type. Expected: p2pkh | p2sh | p2wpkh, got ${type}`,
        );
      }
    }
  }
  ...
  getBlindedPaymentCode(
    ...
    blindedPaymentCode.set(
      utils.xorUint8Arrays(
        s.subarray(0, 32),
        paymentCodeBuffer.subarray(3, 35),
      ),
      3,
    );
    blindedPaymentCode.set(
      utils.xorUint8Arrays(
        s.subarray(32, 64),
        paymentCodeBuffer.subarray(35, 67),
      ),
      35,
    );
...
export class PaymentCodePrivate extends PaymentCodePublic {
  ...
  getPaymentCodeFromNotificationTransactionData(
    ...
    paymentCodeBuffer.set(
      utils.xorUint8Arrays(
        s.subarray(0, 32),
        blindedPaymentCode.subarray(3, 35),
      ),
      3,
    );
    paymentCodeBuffer.set(
      utils.xorUint8Arrays(
        s.subarray(32, 64),
        blindedPaymentCode.subarray(35, 67),
      ),
      35,
    );
...
export const BIP47Factory = (ecc: TinySecp256k1Interface) => {
  ...
  const fromSeed = (
    ...
    const coinType =
      network.pubKeyHash === utils.networks.bitcoin.pubKeyHash ? "0" : "1";
  ...
  const fromBase58 = (
    ...
    const version = buf[0];
    if (version !== PC_VERSION) throw new Error("Invalid version");
