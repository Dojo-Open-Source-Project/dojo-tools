...
const invalidVersionPcode = {
  pcBase58:
    "QM8TJTLJbPRGxSbc8EJi42Wrr6QbNSaSSVJ5Y3E4pbCYiTHUskHg13935Ubb7q8tx9GVbh2UuRnBc3WSyJHhUrw8KhprKnn9eDznYGieTzFcwQRya4GA",
};
...
const DESIGNATED_PUBKEY =
  "0272d83d8a1fa323feab1c085157a0791b46eba34afb8bfbfaeb3a3fcc3f2c9ad8";
const DESIGNATED_PRIVKEY =
  "Kx983SRhAZpAhj7Aac1wUXMJ6XZeyJKqCxJJ49dxEbYCT4a1ozRD";
const ALICE_BLINDED_PCODE =
...
describe("bip47", () => {
  describe("fromBase58()", () => {
    it("should successfully initialize a PaymentCode from a base58 payment code", () => {
      ...
      assert.strictEqual(
        bytesToHex(alicePcode.getNotificationPublicKey()),
        alice.notifPubKey,
      );
...
describe("PaymentCode", () => {
  ...
  describe("PaymentCodePublic.clone()", () => {
    it("should successfully clone a public payment code", () => {
      ...
      assert.notEqual(
        alicePcodeClone.paymentCode.buffer,
        alicePcode.paymentCode.buffer,
      );
      assert.deepStrictEqual(
        alicePcodeClone.paymentCode,
        alicePcode.paymentCode,
      );
    });
  });
  ...
  describe("PaymentCodePrivate.clone()", () => {
    it("should successfully clone a private payment code", () => {
      ...
      assert.deepStrictEqual(
        alicePcodeClone.root.privateKey,
        alicePcode.root.privateKey,
      );
    });
  });
  ...
  describe("PaymentCode.derivePaymentPublicKey()", () => {
    it("should successfully derive public keys from a payment code and a notif privkey", () => {
      ...
      assert.strictEqual(
        bytesToHex(pubkeyPayment),
        aliceToBobAddresses[0].pubkey,
      );
    });
  ...
  describe("PaymentCode.derivePaymentPrivateKey()", () => {
    it("should successfully derive private keys from a payment code and a notif pubkey", () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        const strPubkeyPayment = (
          ecc.pointFromScalar(privkeyPayment) ?? ""
        ).toString();
        const strPubkeyPayment2 = bobPcode
          .derivePaymentPublicKey(alicePcode, i)
          .toString();
  ...
  describe("PaymentCodePrivate.getPaymentCodeFromNotificationTransactionData()", () => {
    ...
    it("should successfully get Alice payment code from a notification transaction data", () => {
      const alicePcode = bobPcode.getPaymentCodeFromNotificationTransactionData(
        hexToBytes(OP_RETURN_DATA),
        hexToBytes(OUTPOINT),
        hexToBytes(DESIGNATED_PUBKEY),
      );
    ...
    it("throws an error on invalid OP_RETURN", () => {
      assert.throws(
        () =>
          bobPcode.getPaymentCodeFromNotificationTransactionData(
            hexToBytes("abababab"),
            hexToBytes(OUTPOINT),
            hexToBytes(DESIGNATED_PUBKEY),
          ),
  ...
  describe("PaymentCodePublic.getBlindedPaymentCode()", () => {
    ...
    it("should successfully blind Alice payyment code for Bob", () => {
      ...
      const blindedPcode = alicePcode.getBlindedPaymentCode(
        bobPcode,
        hexToBytes(OUTPOINT),
        privKey,
      );
