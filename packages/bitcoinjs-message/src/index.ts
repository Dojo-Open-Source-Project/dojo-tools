...
import { type TinySecp256k1Interface, testEcc } from "./testecc.js";
...
function encodeSignature(
  ...
  recovery: number,
  compressed?: boolean,
  segwitType?: (typeof SEGWIT_TYPES)[keyof typeof SEGWIT_TYPES],
): Uint8Array {
...
export function magicHash(
  ...
  messagePrefix?: string | null | Uint8Array,
): Uint8Array {
...
function prepareSign(
  messagePrefix?: string | Uint8Array | SignatureOptions,
  sigOptions?: SignatureOptions,
): PrepareSignReturnType {
  let messagePrefixArg: string | Uint8Array | undefined;
  if (
    typeof messagePrefix === "object" &&
    sigOptions === undefined &&
    !isBytes(messagePrefix)
  ) {
  ...
  if (segwitType && typeof segwitType === "string") {
    segwitType =
      segwitType.toLowerCase() as (typeof SEGWIT_TYPES)[keyof typeof SEGWIT_TYPES];
  }
  if (
    segwitType &&
...
function segwitRedeemHash(publicKeyHash: Uint8Array): Uint8Array {
  const redeemScript = concatBytes(hexToBytes("0014"), publicKeyHash);
