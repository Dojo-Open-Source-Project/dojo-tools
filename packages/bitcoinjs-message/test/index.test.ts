...
fixtures.valid.sign.forEach((f) => {
  test("sign: " + f.description, async () => {
    ...
    const signer = (hash: Uint8Array, ex?: Uint8Array) =>
      secp256k1.signRecoverable(hash, pk, ex);
    const signerAsync = async (hash: Uint8Array, ex?: Uint8Array) =>
      secp256k1.signRecoverable(hash, pk, ex);
...
test("Check that compressed signatures can be verified as segwit", () => {
  ...
  assert.isTrue(
    message.verify(msg, p2pkhAddress.address!, signature, null, true),
  );

  // Check segwit addresses with true
  assert.isTrue(
    message.verify(msg, p2shp2wpkhAddress.address!, signature, null, true),
  );
  assert.isTrue(
    message.verify(msg, p2wpkhAddress.address!, signature, null, true),
  );
