...
import { FeeEstimator, type Options } from "@dojo-tools/next-block-estimator";
...
const main = async () => {
  ...
  if (
    values.mode &&
    !(modeOptions as readonly string[]).includes(values.mode)
  ) {
    throw new Error(
  ...
  const refresh = values.refresh == null ? undefined : Number(values.refresh);
